...
def cmd_run_phase(args: argparse.Namespace) -> None:
    ...
    emit_json_response(
        payload,
        budget=True,
        ledger={"task_id": task_id, "tool": "planner_run_phase"},
    )
...
def cmd_run_task(args: argparse.Namespace) -> None:
    ...
    emit_json_response(
        payload,
        budget=True,
        ledger={"task_id": task_id, "tool": "planner_run_task"},
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_init = sub.add_parser("init", help="Create a new task directory")
    ...
    p_save = sub.add_parser("save", help="Save a workflow artifact")
    ...
    p_run = sub.add_parser("run-phase", help="eval-phase + execute all nodes (cheap tier, no agent in loop)")
    ...
    p_run_task = sub.add_parser("run-task", help="Run all phases sequentially (eval + execute each)")
    ...
    p_status = sub.add_parser("status", help="One-line task status for quick checks")
    ...
    p_replan = sub.add_parser("replan-packet", help="Minimal replan package after escalation")
    p_replan.add_argument("--task-id", required=True)
    p_replan.set_defaults(func=cmd_replan_packet)

    p_patch = sub.add_parser("patch-node", help="Incrementally patch DAG nodes (replace/insert_after/delete)")
    ...
    p_sess_get = sub.add_parser("session-get", help="Read host session pointers + recommended_next")
    p_sess_get.add_argument("--task-id", required=True)
    p_sess_get.set_defaults(func=cmd_session_get)

    p_sess_set = sub.add_parser("session-set", help="Update host session pointers after polling")
